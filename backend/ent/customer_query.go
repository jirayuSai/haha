...
func (cq *CustomerQuery) QueryGenders() *GenderQuery {
...
// QueryPersonals chains the current query on the personals edge.
func (cq *CustomerQuery) QueryPersonals() *PersonalQuery {
...
func (cq *CustomerQuery) QueryTitles() *TitleQuery {
...
func (cq *CustomerQuery) WithGenders(opts ...func(*GenderQuery)) *CustomerQuery {
	query := &GenderQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withGenders = query
	return cq
}

//  WithPersonals tells the query-builder to eager-loads the nodes that are connected to
// the "personals" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CustomerQuery) WithPersonals(opts ...func(*PersonalQuery)) *CustomerQuery {
	query := &PersonalQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withPersonals = query
	return cq
}
...
func (cq *CustomerQuery) WithTitles(opts ...func(*TitleQuery)) *CustomerQuery {
	query := &TitleQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	var (
		...
		loadedTypes = [3]bool{
			cq.withGenders != nil,
			cq.withPersonals != nil,
			cq.withTitles != nil,
		}
	)
	if cq.withGenders != nil || cq.withPersonals != nil || cq.withTitles != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withGenders; query != nil {
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.Genders = n
			}
		}
	}

	if query := cq.withPersonals; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Customer)
		for i := range nodes {
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.Personals = n
			}
		}
	}

	if query := cq.withTitles; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Customer)
		for i := range nodes {
