...
func (gq *GenderQuery) QueryCustomers() *CustomerQuery {
...
func (gq *GenderQuery) WithCustomers(opts ...func(*CustomerQuery)) *GenderQuery {
	query := &CustomerQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withCustomers = query
	return gq
}
...
func (gq *GenderQuery) sqlAll(ctx context.Context) ([]*Gender, error) {
	...
	if query := gq.withCustomers; query != nil {
		...
		query.Where(predicate.Customer(func(s *sql.Selector) {
			s.Where(sql.InValues(gender.CustomersColumn, fks...))
		}))
