...
func (tq *TitleQuery) QueryCustomers() *CustomerQuery {
...
//  WithCustomers tells the query-builder to eager-loads the nodes that are connected to
// the "customers" edge. The optional arguments used to configure the query builder of the edge.
func (tq *TitleQuery) WithCustomers(opts ...func(*CustomerQuery)) *TitleQuery {
	query := &CustomerQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withCustomers = query
	return tq
}
...
func (tq *TitleQuery) sqlAll(ctx context.Context) ([]*Title, error) {
	...
	if query := tq.withCustomers; query != nil {
		...
		query.Where(predicate.Customer(func(s *sql.Selector) {
			s.Where(sql.InValues(title.CustomersColumn, fks...))
		}))
