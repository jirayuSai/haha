...
func (tu *TitleUpdate) AddCustomers(c ...*Customer) *TitleUpdate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
...
func (tu *TitleUpdate) RemoveCustomers(c ...*Customer) *TitleUpdate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
...
func (tu *TitleUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if nodes := tu.mutation.RemovedCustomersIDs(); len(nodes) > 0 {
	...
	if nodes := tu.mutation.CustomersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   title.CustomersTable,
...
// AddCustomers adds the customers edges to Customer.
func (tuo *TitleUpdateOne) AddCustomers(c ...*Customer) *TitleUpdateOne {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
...
// RemoveCustomers removes customers edges to Customer.
func (tuo *TitleUpdateOne) RemoveCustomers(c ...*Customer) *TitleUpdateOne {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
...
func (tuo *TitleUpdateOne) sqlSave(ctx context.Context) (t *Title, err error) {
	...
	if nodes := tuo.mutation.RemovedCustomersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   title.CustomersTable,
	...
	if nodes := tuo.mutation.CustomersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   title.CustomersTable,
